...
def _result(
    rule_id: str, level: str, message: str, artifact: str, file: str = "", line: int = 0
) -> dict:
...
def to_sarif(report: ScanReport) -> dict:
    ...
    if t and t.is_typosquat:
        results.append(
            _result(
                ...
                f"'{t.similar_package}'"
                + (f" (afijos: {', '.join(t.suffixes)})" if t.has_combo_affix else "")
                + ".",
                artifact,
            )
        )
            ...
            for f in nw:
                results.append(
                    _result(
                        "PS005",
                        "note",
                        f"Literal de red embebido: {f.name}.",
                        artifact,
                        file=f.file,
                        line=f.line,
                    )
    ...
    if report.suggestion:
        results.append(
            _result(
                "PS200", "note", f"Alternativa legítima sugerida: '{report.suggestion}'.", artifact
            )
        )
