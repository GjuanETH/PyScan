...
from pyscan.classifier import (ModelNotAvailable, feature_names, load_bundle,
                               predict)
...
def _malicious_fv() -> FeatureVector:
    return FeatureVector(name_min_distance=1.0, is_typosquat=1.0,
                         entropy_max=7.8, entropy_suspicious_windows=12.0,
                         ast_dangerous_calls=6.0, ast_has_install_hook=1.0,
                         ast_network_literals=4.0)


def _benign_fv() -> FeatureVector:
    return FeatureVector(name_min_distance=99.0, release_count=50.0,
                         requires_count=5.0, has_long_description=1.0,
                         entropy_max=5.2, ast_dangerous_calls=0.0)
...
def _train_bundle(tmp_path: Path) -> Path:
    ...
    rows, labels = [], []
    for _ in range(20):
        rows.append([_malicious_fv().to_row()[n] for n in names]); labels.append(1)
        rows.append([_benign_fv().to_row()[n] for n in names]); labels.append(0)
